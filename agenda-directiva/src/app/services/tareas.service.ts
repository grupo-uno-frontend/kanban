...
import { addDoc, collection, deleteDoc, doc, limit, orderBy, query, setDoc, where } from 'firebase/firestore';
...
export class TareasService {
  ...
  getTareas(usuario: string): Observable<ITarea[]> {
    ...
    const tareasDisponibles = query(tareasUsuario, where('realizado', '==', false));
    /* const realizadas = query(tareas, orderBy('fechaHora', 'asc'), limit(20));//desc */
    return collectionData(tareasDisponibles, { idField: 'id' }) as Observable<ITarea[]>;}

  getTareasFinalizadas(usuario: string): Observable<ITarea[]> {
    const tareas = collection(this.firestore, 'tareas');
    const tareasUsuario = query(tareas, where('usuario', '==', usuario));
    const tareasDisponibles = query(tareasUsuario, where('realizado', '==', true));
    /* const realizadas = query(tareas, orderBy('fechaHora', 'asc'), limit(20));//desc */
    return collectionData(tareasDisponibles, { idField: 'id' }) as Observable<ITarea[]>;}
